...
// REGISTRO DE LA TOOL: Le enseñamos al servidor a listar issues abiertos
server.tool(
    "list-issues",
    ListIssuesSchema.shape, // Desempaquetamos el molde con .shape
    async (args) => {
        try {
            const { owner, repo } = args;
            ...
            // Llamamos a Octokit para traer los issues
            const response = await octokit.rest.issues.listForRepo({
                owner,
                repo,
                state: "open", // Traemos únicamente los que están abiertos
            ...
            if (error.status === 404) {
                mensajeAmigable = `No se encontró el repositorio "${args.owner}/${args.repo}". Verificá los datos e intentá de nuevo.`;
            }
